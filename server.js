...
const dataBot = {
    ...
    spawn() {
	    let newbotNick = bots.name[~~(Math.random() * bots.name.length)];
            ...
            case 0:
                if (!user.startedBots) {
                    ...
                    startBotsInterval = setInterval(() => {
                        if (dataBot.lastPlayersAmount < 202 && connectedBots < bots.amount && !stoppingBots) userBots.push(new Bot())
                    }, 200)
                ...
                if (user.startedBots && !stoppingBots) {
                    ...
                    setInterval(() => {
                        if (seconds === 10) {
                            ...
                            logger.warn(`[SERVER] Stopping bots in ${10 - seconds} seconds`);
                            seconds++
                        }
                    }, 1000)
                }
